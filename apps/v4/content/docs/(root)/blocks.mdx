...
我们邀请社区一同完善 [Blocks 库](/blocks)，分享你的组件与区块，帮助更多开发者构建高质量、可复用的组件集合。

欢迎提交各种类型的区块：应用、营销页、产品展示等都可以。
...
区块可以是单个组件（例如某个 UI 组件的变体），也可以是复杂组件（如仪表盘）包含多个组件、hook 和工具函数。
...
### 在 `registry-blocks.tsx` 中注册区块

要让区块出现在注册表中，需要在 `registry-blocks.ts` 中新增定义。其结构遵循 [https://ui.shadcn.com/schema/registry-item.json](https://ui.shadcn.com/schema/registry-item.json)。
...
<Callout className="mt-6">
  **提示：** 不必每次改动都运行构建脚本，仅在更新区块定义时执行即可。
</Callout>

### 预览区块

脚本完成后，可在 `http://localhost:3333/blocks/[CATEGORY]` 查看区块，或访问 `http://localhost:3333/view/styles/new-york/dashboard-01` 获得全屏预览。
...
在区块目录中编辑文件，即可实时在浏览器中查看效果。如果增加了文件，记得同步更新区块定义里的 `files` 数组。
...
提交你的改动并发起 PR。区块通过审核后会合并到主仓库，同时发布到网站并可通过 CLI 安装。

</Steps>

## 分类

`categories` 属性用于在注册表中对区块进行分类。
...
- 区块定义必须包含：`name`、`description`、`type`、`files`、`categories`。
- `registryDependencies` 中应列出所有依赖的注册表组件（如 `input`、`button`、`card` 等）。
- `dependencies` 中应列出所有外部包依赖（如 `zod`、`sonner` 等）。
- 如果区块包含页面（可选），应在 `files` 数组首位，且带有 `target` 字段，方便 CLI 正确放置页面文件。
